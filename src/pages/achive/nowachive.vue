    		...
    		<panel :paneltitle="paneltitle">
    			<ul class="achive_info">
    				<li v-for="item in achive_list "  :class="{'finish':item.status == 3,'status_one':item.status==1,'status_two':item.status==2}">
    					...
    					<span  @click="achiveStar(item.achieve_id)"><i v-if='item.status == 3'>已达成</i></span>
    				</li>
    			</ul>
    		</panel>
    	</div>
  ...
  export default {
    ...
    methods: {
        init(width, height) { 
            ...
            service.getListData('today' , this.updateListData , this.updateError );
            this.initTodayStars();
        },
        ...
        updateListData(data) {
           ...
           if (data.http_code == 200) {
              this.achive_list = data.data.today_list; 
        ...
        achiveStar(num){
            //点击完成按钮，提交数据，将数据渲染
            service.getAchiveToday(num,this.achiveStarSuccess , this.achiveStarError);
        },
...
<style scoped>
 /*红色 #ef5361 紫色:8c3ffa;  字体颜色： #313131  边框颜色 ：e1e1e1 灰色：#b5b5b5*/
  ...
  .achive_status .health-header .tip { margin-top: -0.6rem; font-size: .8rem; }
  ...
  .today_achive .achive_info li span:nth-child(1){padding:0 3%; font-family:AkzidenzGrotesk;  font-size: 1.4rem; color:#8c3ffa; border-right:1px solid #e1e1e1; position:relative; bottom:6px;  width:18%; text-align: center;}
  ...
  .today_achive .achive_info li span:nth-child(3){
  	background:#ebebeb url(~assets/images/btn_20_band_award_dis@3x.png) no-repeat; background-size: 45px 20px; border-radius:15px; text-align: center; font-size: .1rem; color:#8c3ffa; float:right; position:relative; top:12px; width:45px; height:20px;}
